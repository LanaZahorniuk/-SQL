...
-- UNIQUE - значение в поле должно быть уникальное
...
insert into users
values (1, 'Lana', 'Carl', 20, 'lanacarl@gmail.com'),
(2, 'Christian', 'Carl', 25, 'christcarl@gmail.com');
...
values ('Lana', 'F', 20, 'lanacarl@gmail.com');

select * from workers;

insert into workers(first_name, gender, age, email)
values ('Lisa', 'F', 19, 'lisa@gmail.com');
...
select firstname, lastname, age, has_child from staff
...
-- Найти список работников, которым меньше 25, 
-- у них младший уровень должности(Junior) и у них нет детей
select * from staff
where age < 25 and position like 'Junior%' and has_child = 'N';

-- Найти список работников с именем Анна, 
-- у которых уровень должности либо средний(Middle), либо старший(Senior)
select * from staff
...
-- drop database имя_бд; - удалить бд
...
alter table products
add price int default 0; -- по умолчанию price = 0

-- Удалить столбец quality
alter table products
drop quality;

-- Изменить тип price на decimal
alter table products
...
-- Добавить новое поле count тип int и удалить поле quantity
alter table products
add count int,
drop quantity;
